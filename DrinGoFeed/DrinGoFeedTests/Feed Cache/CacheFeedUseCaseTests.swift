...
class LocalFeedLoader {
    ...
    func save(_ items: [CocktailItem], completion: @escaping (Error?) -> Void) {
    ...
    private func cache(_ items: [CocktailItem], with completion: @escaping (Error?) -> Void) {
...
class CacheFeedUseCaseTests: XCTestCase {
    ...
    func test_save_doesNotDeliverDeletionErrorAfterSUTInstanceHasBeenDeallocated() {
        ...
        sut?.save([uniqueItem(idx: 0)], completion: { receivedResult.append($0) })
    ...
    private func expect(_ sut: LocalFeedLoader, toCompleteWithError expectedError: NSError?, when action: () -> Void, file: StaticString = #filePath, line: UInt = #line) {
        ...
        sut.save([uniqueItem(idx: 0)]) { error in
            receivedError = error
            exp.fulfill()
        }
